...
<pre>
  Devise.setup do |config|
    ...
		config.rpx_auto_create_or_merge_account = true # false if you don't want to create or merge user accounts with identifiers automaticaly. True by default.
...
There are two hooks that you may define in your model : @before_rpx_success@ (called before each successful connection) and @before_rpx_auto_create@ (called only once before the auto saving of the user instance, when rpx_auto_create_or_merge_account is enabled).
